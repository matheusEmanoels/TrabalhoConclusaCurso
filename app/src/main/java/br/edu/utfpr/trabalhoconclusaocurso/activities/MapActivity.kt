...
class MapActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnIniciarParar.setOnClickListener { onClickIniciarParar() }
        btnFinalizar.setOnClickListener { onClickFinalizar() }

        val mapFragment = supportFragmentManager.findFragmentById(R.id.map) as SupportMapFragment
        mapFragment.getMapAsync(this)

        val usuario = SessaoUsuario.getUsuario()
        usuario?.let { Toast.makeText(this, "Bem-vindo, ${it.username}!", Toast.LENGTH_SHORT).show() }
    ...
    private fun onClickIniciarParar() {
        if (isTracking) {
            // Pausar
            btnIniciarParar.text = "Retomar"
            btnIniciarParar.setBackgroundColor(getColor(R.color.success))
            enviarAcaoService("PAUSE")
        } else {
            // Iniciar ou Retomar
            btnIniciarParar.text = "Pausar"
            btnIniciarParar.setBackgroundColor(getColor(R.color.error))
            if (!::atividadeId.isInitialized || atividadeId.isEmpty()) {
                atividadeId = UUID.randomUUID().toString()
            }
            enviarAcaoService("START")
    ...
    private fun onClickFinalizar() {
        if (isTracking) {
            ...
            isTracking = false
            salvarFlag(this, false)
        }
        // Mostra o resumo e oculta botões de controle
    ...
    private fun enviarAcaoService(acao: String) {
        val serviceIntent = Intent(this, LocationService::class.java)
        serviceIntent.putExtra("atividadeId", atividadeId)
        serviceIntent.putExtra("action", acao)
    ...
    override fun onResume() {
        ...
        ContextCompat.registerReceiver(this, locationReceiver, locationFilter, ContextCompat.RECEIVER_EXPORTED)
        val finalFilter = IntentFilter("FINAL_UPDATE")
        ContextCompat.registerReceiver(this, finalReceiver, finalFilter, ContextCompat.RECEIVER_EXPORTED)
    }
    ...
    private val finalReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (pontosPercurso.isNotEmpty()) {
                ...
                pontosPercurso.forEach { boundsBuilder.include(it) }
    ...
    private fun atualizarResumo(intent: Intent) {
    ...
    private fun carregarHistorico(atividadeId: String) {
        ...
        coordenadas.forEach {
            val ponto = LatLng(it.latitude, it.longitude)
            polylineOptions.add(ponto)
            pontosPercurso.add(ponto)
        }
        ...
        atividade?.let {
            val pace = if (it.distancia > 0) (it.duracao / 60.0) / (it.distancia / 1000.0) else 0.0
            ...
            val horas = (it.duracao / 3600).toInt()
            val minutos = ((it.duracao % 3600) / 60).toInt()
            val segundos = (it.duracao % 60).toInt()
            tvDuracao.text = "Duração: %02d:%02d:%02d".format(horas, minutos, segundos)
    ...
    private fun salvarFlag(context: Context, isTracking: Boolean) {
    ...
    private fun carregarFlag(context: Context): Boolean {
        val sharedPref = context.getSharedPreferences("IsTracking", Context.MODE_PRIVATE)
